...
Objective
    ...
    problem with a direct inverse ML approach. For the FE data produced with
    Gent material model.
...
def PlotS():
    ...
    plt.plot(ytest[:,1],Yhat_test[:,1],'ob',label='Scaled Jm, $R^2=$ %0.7f'%(R2[0]), alpha = 0.5);
    plt.plot(ytest[:,0],Yhat_test[:,0],'ok',label='Scaled $\mu$, $R^2=$ %0.7f'%(R2[0]), alpha = 0.5);
    plt.xlabel('Target Material Parameter'); plt.ylabel('Predicted Material Parameter')
